...
Filters a colored point cloud by HSV (using the packed `rgb` field), applies
a workspace box, and publishes the centroid as `/ball_pose`.
"""
...
class BallDetector(Node):
    ...
    def _on_cloud(self, msg: PointCloud2):
        ...
        if not np.any(color_mask):
            self.get_logger().warn('no points survived HSV filter', throttle_duration_sec=2.0)
            return
        ...
        n = int(ws_mask.sum())
        if n <= 0:
            self.get_logger().warn('points matched HSV but not workspace box',
                                   throttle_duration_sec=2.0)
            return
        if n < self.min_color_points:
            self.get_logger().warn(f"only {n} points (< {self.min_color_points})",
                                   throttle_duration_sec=2.0)
            return

        self._publish(points[ws_mask], msg.header.stamp)
    ...
    def _on_parameter_update(self, params):
        ...
        for p in params:
            if p.name in bounds and p.type_ == Parameter.Type.DOUBLE:
                ...
                if int(p.value) < 1:
                    return SetParametersResult(successful=False, reason='cloud_stride must be >= 1')
...
def _run_tuner(topic: str) -> int:
    ...
    node.get_logger().info(f'hsv_tuner subscribed to {topic}')
