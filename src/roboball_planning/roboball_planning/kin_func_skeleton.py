...
def skew_3d(omega):
    ...
    if omega.shape != (3,) and omega.shape != (3, 1):
...
def rotation_3d(omega, theta):
    ...
    omega = omega.flatten() / norm
    omega_hat = skew_3d(omega)
    angle = norm * theta
    return (np.eye(3) 
            + np.sin(angle) * omega_hat 
            + (1 - np.cos(angle)) * omega_hat @ omega_hat)
...
def hat_3d(xi):
    ...
    if xi.shape != (6,) and xi.shape != (6, 1):
...
def homog_3d(xi, theta):
    ...
    angle = norm * theta
    w_hat = skew_3d(w)
    R = np.eye(3) + np.sin(angle) * w_hat + (1 - np.cos(angle)) * w_hat @ w_hat

    # Normalize v by ||omega|| since the formula assumes unit omega
    v_norm = v / norm
    p = (np.eye(3) - R) @ (w_hat @ v_norm) + np.outer(w, w) @ v_norm * angle
...
def prod_exp(xi, theta):
    ...
    if not xi.shape[0] == 6:
        raise TypeError('xi must have 6 rows')
    if not xi.shape[1] == theta.shape[0]:
        raise TypeError('xi and theta must have matching columns/length')
